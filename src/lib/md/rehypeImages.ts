...
function transform(
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (node.type === 'element' && node.tagName === 'img') {
      ...
      if (/^[a-z]:[\\/]/i.test(src)) {
        children[i] = { type: 'element', tagName: 'span', properties: { className: ['missing-legacy-image'] }, children: [{ type: 'text', value: '原文此处图片来自旧电脑，文件暂缺。' }] }
        continue
      }
      if (src && !src.startsWith('data:')) {
        const title = props.title != null ? String(props.title) : ''
        const alt = props.alt != null ? String(props.alt) : ''
        const finalSrc = (legacyImages as Record<string, string>)[src] || resolveContentImage(src, filePath) || src
