...
构建不会通过文件修改时间自动更新文章日期，也不会自动写入 slug。只有日期的字段只显示日期；包含时间的字段按北京时间显示到分钟，包括真实的零点。HTML 的 `datetime`、文章元数据也保留原始精度。保持原始 Markdown 对外可读，以兼容发布服务的内容哈希验证。

新建文章可运行 `node scripts/new-post.js article-id`，文件写入实际使用的 `public/markdown/`，`published` 自动记录带时区的完整时间。发布服务也应在创建时写入 `published: "2026-09-18T14:30:00+08:00"`，编辑时保留该字段并写入 `updated_at: "2026-09-19T09:15:00+08:00"`。旧接口仍可只提供 `date: "2026-09-18"`，此时只展示日期，不能恢复未记录的时分。未标注时区的完整时间按北京时间读取。
...
文章 frontmatter 的 `index` 控制置顶：0（默认）不置顶，正整数置顶，数字越大越靠前。飞书助手支持在完整预览并确认后修改此值。修改文章（包括分类、标签及置顶状态）记录北京时间 `updated_at`；卡片与详情页显示最近更新时间，原发布日期保持不变。卡片内部显示发布时间，左下角在没有更新记录时显示首次发布时间，有更新记录后显示最近更新时间；两种情况使用相同图标和样式，不加区分提示。日期和完整时间均保留原有精度。没有更新记录时，详情页和结构化数据不使用发布时间冒充更新时间。
